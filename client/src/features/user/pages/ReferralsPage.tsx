  ...
  return (
    <div className="p-4 md:p-8 space-y-8 max-w-7xl mx-auto w-full pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
           <h1 className="text-3xl font-black text-gray-900 tracking-tight uppercase text-emerald-600">SOCIAL IMPACT</h1>
      ...
      <Card className="p-10 bg-emerald-950 text-white relative overflow-hidden shadow-2xl">
         ...
         <div className="relative z-10 text-center space-y-6 max-w-3xl mx-auto">
            <h2 className="text-3xl font-black tracking-tight leading-loose uppercase">GROW THE FOREST<br /><span className="text-emerald-500 underline decoration-wavy">TOGETHER</span></h2>
            ...
            <div className="max-w-md mx-auto pt-6 space-y-4">
               <div className="flex gap-2">
                  <Input value={referralCode} readOnly className="h-14 text-center text-lg font-black bg-white/5 border-white/20 text-white placeholder-white/40" />
                  <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                     <Button onClick={handleCopyCode} className="h-14 px-8 bg-emerald-600 text-white font-black uppercase tracking-widest hover:bg-emerald-500 transition-all shadow-lg shadow-emerald-900/40">
                       COPY
                     </Button>
                  </motion.div>
               </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <Card className="p-8 border-none shadow-sm space-y-8 bg-white min-h-[500px]">
                  <div className="flex items-center justify-between">
                     <h3 className="text-xl font-black text-gray-900 uppercase">Referral Network</h3>
                   ...
                   <Button variant="ghost" className="w-full text-[10px] font-black uppercase text-gray-400 tracking-widest mt-auto pt-8">View Complete Network History</Button>
               </Card>
               ...
               <Card className="p-8 border-none shadow-sm space-y-8 bg-white relative overflow-hidden min-h-[500px] flex flex-col">
                  <div className="flex items-center justify-between mb-4">
                     <div>
                        <h2 className="text-3xl font-black text-amber-500 tracking-tighter leading-none">{referralsCount * 7 + 3}-day streak</h2>
                        <p className="text-[11px] font-black text-gray-400 mt-3 uppercase tracking-widest italic opacity-60">You're making a difference!</p>
                     </div>
                  ...
                  <div className="flex justify-between items-center py-6 border-y border-gray-50 bg-gray-50/30 -mx-4 px-4 rounded-xl">
                     {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, idx) => (
                        <div key={idx} className="flex flex-col items-center gap-3">
                           ...
                           <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-xs transition-all ${idx < 3 ? "bg-amber-400 text-white shadow-xl shadow-amber-100 scale-110" : "bg-white border-2 border-gray-100 text-gray-200"}`}>
                              {idx < 3 ? "✓" : ""}
                           </div>
                        </div>
                     ))}
                  ...
                  <div className="flex items-center gap-3 bg-blue-50/50 p-4 rounded-2xl border border-blue-100 mt-4">
                     ...
                     <p className="text-[10px] font-black text-blue-700 uppercase tracking-widest">1 streak freeze ready to use</p>
                  </div>
         ...
         <div className="space-y-8 order-1 lg:order-3">
             <Card className="p-10 border-none shadow-sm space-y-8 bg-emerald-50/30">
                <h4 className="text-sm font-black text-emerald-900 tracking-[0.2em] uppercase">Impact Milestone</h4>
                <div className="space-y-6">
                   <div className="flex justify-between items-end">
                      <div>
                         <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest leading-none mb-1">Rank Booster</p>
                         <h4 className="font-bold text-gray-900">10 Active Referrals</h4>
                      </div>
                      <p className="text-xs font-black text-gray-400 uppercase tracking-[0.2em]">{referralsCount}/10</p>
                   </div>
             ...
             <div className="p-10 space-y-6">
                <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-6">How Ranking Works</h4>
                ...
                <div className="pt-12">
                   <Button 
                      ...
                      className="w-full bg-[#1e293b] text-white font-black text-[12px] uppercase tracking-[0.1em] py-8 rounded-3xl hover:bg-black transition-all shadow-2xl hover:scale-[1.02] active:scale-95"
                   >
  ...
  return (
    <Card className="p-6 border-none shadow-sm space-y-6 flex flex-col items-center text-center group hover:scale-[1.02] transition-transform">
       ...
       <div>
          ...
          <h4 className="text-2xl font-black text-gray-900 tracking-tighter italic">{value}</h4>
       </div>
    </Card>
  );
};
...
const StreakBadge = ({ days, label, achieved }: any) => (
  <div className="flex flex-col items-center gap-1 opacity-80 hover:opacity-100 transition-opacity">
     ...
     <p className={`text-[8px] font-black uppercase tracking-tighter ${achieved ? "text-amber-600" : "text-gray-400"}`}>{days}</p>
...
const ReferralMember = ({ member, index }: any) => (
  <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} className="flex items-center justify-between p-5 bg-gray-50/50 rounded-2xl border border-gray-100 hover:border-emerald-200 transition-colors">
     <div className="flex items-center gap-4">
        <div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center font-black text-emerald-600 text-xs shadow-inner">
           {member.name[0]}
        </div>
        <div>
     ...
     <div className="text-right">
        <p className="font-black text-emerald-600 text-sm italic">{member.impact}</p>
...
const HowMetric = ({ num, label, weight }: any) => (
  <div className="flex items-center justify-between border-b border-gray-50 pb-3">
     <div className="flex gap-4 items-center">
        <span className="text-emerald-100 font-black text-lg leading-none">#{num}</span>
